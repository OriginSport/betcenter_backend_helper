...
def recover_page_url(path_info, request_data, page_name='page'):
    return '%s?%s' % (path_info, '&'.join([d[0] + '=' + url_quote(d[1]) for d in request_data.items() if  d[0] != page_name]))
...
class BasePaginator(object):
    ...
    def _construct_link(self, page):
        if len(self.request.args):
            if len(self.request.args)  == 1 and self.request.args.get('page', None):
                return '?'.join([self.request.path, 'page=%s' % page])
            else:
                return '&'.join([recover_page_url(self.request.path, self.request.args), 'page=%s' % page])
        else:
...
def dict_pagination_response(object_list, request, page, page_size, wrapper=None):
...
def json_pagination_response(object_list, request, page, page_size, wrapper=None):
    return ok_json(dict_pagination_response(object_list, request, page, page_size, wrapper))
